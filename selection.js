...
(function selectionModule(factory) {
  ...
  function Selection(options = {}) {
    ...
    for (let name in defaults) {
      if (this.options.boundaries) {
        this.options.boundarys = this.options.boundaries
      }

      !(name in this.options) && (this.options[name] =
        defaults[name]);
    }
  ...
  Selection.prototype = {
    ...
    _onTapStart(evt) {
      ...
      this._targetBoundary = this._boundarys.find((el) => _intersects(el,
        target));
    ...
    _updatedTouchingElements() {
      ...
      const check = ((node) => {
        if (_dispatchFilterEvent(this, 'selectionFilter', node) !==
          false) {
     ...
     */
    keepSelection() {
      const keep = this._touchedElements.filter(x => !this._selectedStore.includes(
        x));
  ...
  function _intersects(ela, elb) {
    ...
    return a.left + a.width >= b.left && a.left <= b.left + b.width && a.top +
      a.height >= b.top && a.top <= b.top + b.height;
  }
